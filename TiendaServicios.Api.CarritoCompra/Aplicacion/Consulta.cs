...
namespace TiendaServicios.Api.CarritoCompra.Aplicacion
{
    public class Consulta
    {
        ...
        public class Manejador : IRequestHandler<Ejecuta, CarritoDto>
        {
            ...
            {
                var carritoSesion =
                    await _contexto.CarritoSesion.FirstOrDefaultAsync(x => x.CarritoSesionId == request.CarritoSesionId) ??
                    throw new Exception("No se encontró el carrito.");

                var carritoSesionDetalle = await _contexto.CarritoSesionDetalle.Where(x
                    => x.CarritoSesionId == carritoSesion.CarritoSesionId).ToListAsync();
